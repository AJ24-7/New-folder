...
class _OffersScreenState extends State<OffersScreen> with SingleTickerProviderStateMixin {
  ...
  List<Color> _resolveGradient(String? templateId, String type) {
    if (templateId != null && _templateGradients.containsKey(templateId)) {
      return _templateGradients[templateId]!;
    }
    switch (type) {
  ...
  Widget _buildOfferCard(GymOffer offer) {
    ...
    // Active offers: show in template style
    if (isActive) return _buildActiveTemplateOfferCard(offer);

    // Paused / expired: plain card with muted style
    return _buildPlainOfferCard(offer);
  }

  // ── Template-styled card for active offers ───────────────────────────────
  Widget _buildActiveTemplateOfferCard(GymOffer offer) {
    final tid = offer.templateId ?? '';
    final gradients = _resolveGradient(tid, offer.type);
    final isBoldAccent    = tid == 'bold_accent';
    final isMinimalElegant = tid == 'minimal_elegant';
    final isVibrantNeon   = tid == 'vibrant_neon';

    // Actions popup (same as plain card)
    Widget actionsMenu = PopupMenuButton(
      icon: Icon(
        Icons.more_vert,
        color: (isBoldAccent)
            ? const Color(0xFFFF6B6B)
            : (isMinimalElegant || isVibrantNeon)
                ? Colors.white70
                : Colors.white,
        size: 20,
      ),
      itemBuilder: (context) => [
        ...
        PopupMenuItem(
          value: 'pause',
          child: const Row(children: [Icon(Icons.pause, size: 18), SizedBox(width: 8), Text('Pause')]),
        ),
        const PopupMenuItem(
          value: 'delete',
    ...
    if (isBoldAccent) {
      return _buildBoldAccentAdminCard(offer, actionsMenu);
    }
    if (isMinimalElegant) {
      return _buildMinimalElegantAdminCard(offer, actionsMenu);
    }
    if (isVibrantNeon) {
      return _buildVibrantNeonAdminCard(offer, actionsMenu);
    }
    return _buildGradientAdminCard(offer, gradients, actionsMenu);
  }

  // gradient-based active card
  Widget _buildGradientAdminCard(
      GymOffer offer, List<Color> gradients, Widget actionsMenu) {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: [
            BoxShadow(
              color: gradients.first.withOpacity(0.45),
              blurRadius: 16,
              ...
              Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    // coupon + usage
                    if (offer.couponCode != null || offer.maxUses != null) ...[
                      const SizedBox(height: 8),
                      Row(children: [
                        ...
                        if (offer.maxUses != null) ...[
                          Icon(Icons.people, color: fgMuted, size: 13),
                          const SizedBox(width: 4),
                          Text(offer.usageStatus,
                              style:
                                  TextStyle(color: fgMuted, fontSize: 11)),
                        ],
                      ]),
                    ],
                  ],
  ...
  // bold_accent active card
  Widget _buildBoldAccentAdminCard(GymOffer offer, Widget actionsMenu) {
                ...
                children: [
                  ...
                  if (offer.features.isNotEmpty) ...[
                    ...
                    Wrap(
                      ...
                      children: offer.features.take(4).map((f) => Container(
                            padding: const EdgeInsets.symmetric(
                                horizontal: 8, vertical: 3),
                            decoration: BoxDecoration(
                  ...
                  Row(children: [
                    ...
                    Text(offer.categoryDisplay,
                        style: TextStyle(
                            ...
                            fontWeight: FontWeight.w600)),
                    const SizedBox(width: 12),
                    Icon(Icons.timer, color: Colors.grey[500], size: 12),
                    const SizedBox(width: 4),
                    Expanded(
                      child: Text(offer.remainingDays,
                          style: TextStyle(
  ...
  // minimal_elegant active card
  Widget _buildMinimalElegantAdminCard(GymOffer offer, Widget actionsMenu) {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Padding(
          padding: const EdgeInsets.fromLTRB(22, 14, 8, 18),
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Container(
  ...
  // vibrant_neon active card
  Widget _buildVibrantNeonAdminCard(GymOffer offer, Widget actionsMenu) {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: [
            BoxShadow(
                color: neon1.withOpacity(0.35), blurRadius: 20, spreadRadius: 0),
            BoxShadow(
                color: neon2.withOpacity(0.2), blurRadius: 20, spreadRadius: 0),
          ],
        ),
        child: Padding(
          ...
          child: Column(
            ...
            children: [
                ...
                if (offer.couponCode != null)
                  Container(
                    ...
                    padding: const EdgeInsets.symmetric(
                        horizontal: 8, vertical: 4),
                    decoration: BoxDecoration(
                      color: neon1.withOpacity(0.1),
                      borderRadius: BorderRadius.circular(8),
                      border:
                          Border.all(color: neon1.withOpacity(0.6), width: 1),
                    ),
                    child: Text(offer.couponCode!,
                        style: TextStyle(
                            color: neon1,
                ...
                Wrap(
                  ...
                  children: offer.features.take(4).map((f) => Container(
                        padding: const EdgeInsets.symmetric(
                            horizontal: 8, vertical: 3),
                        decoration: BoxDecoration(
                          color: neon1.withOpacity(0.08),
                          borderRadius: BorderRadius.circular(8),
                          border:
                              Border.all(color: neon1.withOpacity(0.3)),
                        ),
                        child: Text(f,
                            style: TextStyle(
                                color: neon1.withOpacity(0.9),
                                fontSize: 11,
                                fontWeight: FontWeight.w500)),
                      )).toList(),
                ),
              ...
              Row(children: [
                Icon(offer.categoryIcon,
                    color: neon1.withOpacity(0.7), size: 13),
                const SizedBox(width: 4),
                Text(offer.categoryDisplay,
                    style: TextStyle(
                        color: neon1.withOpacity(0.8),
                        fontSize: 11,
                        fontWeight: FontWeight.w600)),
                const SizedBox(width: 12),
                Icon(Icons.timer, color: Colors.white30, size: 12),
  ...
  // ── Plain card for paused / expired offers ───────────────────────────────
  Widget _buildPlainOfferCard(GymOffer offer) {
    return Card(
      ...
      child: InkWell(
        ...
        child: Opacity(
          opacity: offer.status == 'expired' ? 0.65 : 1.0,
          child: Padding(
            padding: const EdgeInsets.all(16),
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
                ...
                Row(
                  children: [
                    ...
                    PopupMenuButton(
                      ...
                      onSelected: (value) =>
                          _handleOfferAction(offer, value.toString()),
                    ),
                  ],
                ),
                const SizedBox(height: 12),
